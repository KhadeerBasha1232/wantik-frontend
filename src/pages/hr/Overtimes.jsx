...
const Overtimes = ({ staff_id }) => {
  ...
  const calculateDuration = (startTime, endTime, otDate) => {
    if (!startTime || !endTime || !otDate) return "";
    const start = new Date(`${otDate}T${startTime}`);
    let end = new Date(`${otDate}T${endTime}`);
    if (end < start) {
      end = new Date(end.getTime() + 24 * 60 * 60 * 1000); // Add one day
    }
  ...
  useEffect(() => {
    if (!token) {
      setError("Please log in to view overtime requests.");
      return;
    }
    ...
    if (staff_id) {
      ...
      // Fetch overtime requests for specific staff
      axios
        .get(`http://127.0.0.1:8000/hr/${type}/overtimes/?staff_id=${staff_id}`, {
          headers: { Authorization: `Bearer ${token}` },
        })
        .then((res) => {
          console.log("Overtime Requests:", res.data);
          const normalizedData = res.data.map((request) => ({
            ...request,
            ot_date: normalizeDate(request.ot_date),
            request_date: normalizeDate(request.request_date),
          }));
          setOvertimeRequests(normalizedData);
          setFilteredRequests(normalizedData);
        })
        .catch((err) => {
          console.error("Error fetching overtime requests:", err.response?.data);
      ...
      axios
        .get(`http://127.0.0.1:8000/hr/${type}/overtimes/`, {
          headers: { Authorization: `Bearer ${token}` },
        })
        .then((res) => {
          console.log("Overtime Requests:", res.data);
  ...
  useEffect(() => {
    setFilteredRequests(
      overtimeRequests.filter((request) => {
        ...
        const filterDate = filters.requestDate ? new Date(filters.requestDate) : null;
        const otDate = new Date(request.ot_date);
        const filterOtDate = filters.otDate ? new Date(filters.otDate) : null;

        return (
          (!filters.staffName ||
            request.staff_name.toLowerCase().includes(filters.staffName.toLowerCase())) &&
          (!filters.year ||
            requestDate.getFullYear().toString() === filters.year) &&
          (!filters.requestDate ||
            request.request_date === filters.requestDate) &&
          (!filters.otDate || request.ot_date === filters.otDate)
        );
      })
    );
  }, [filters, overtimeRequests]);
  ...
  const deleteComment = async (index) => {
    ...
    if (comment.id) {
      try {
        await axios.delete(
          `http://127.0.0.1:8000/hr/${type}/overtimes/${selectedRequest.id}/comments/${comment.id}/`,
          { headers: { Authorization: `Bearer ${token}` } }
        );
  ...
  const handleStatusChange = async (requestId, field, value) => {
    ...
    try {
      const response = await axios.patch(
        `http://127.0.0.1:8000/hr/${type}/overtimes/${requestId}/`,
        { [field]: value },
        { headers: { Authorization: `Bearer ${token}` } }
      );
        ...
        prev.map((req) =>
          req.id === requestId
            ? {
                ...req,
                [field]: value,
                ot_date: normalizeDate(response.data.ot_date),
  ...
  const handleSubmitNewOvertime = async (e) => {
    e.preventDefault();
    if (!token) {
      setError("Please log in to submit an overtime request.");
      return;
    }

    if (!overtimeForm.staff_id) {
      setError("Please select a staff member.");
      return;
    }

    if (!overtimeForm.ot_date) {
      setError("Overtime date is required.");
      return;
    }

    if (!overtimeForm.ot_start_time || !overtimeForm.ot_end_time) {
      setError("Start and end times are required.");
      return;
    }

    const start = new Date(`${overtimeForm.ot_date}T${overtimeForm.ot_start_time}`);
    let end = new Date(`${overtimeForm.ot_date}T${overtimeForm.ot_end_time}`);
    if (end < start) {
    ...
    const requestData = {
      staff_id: overtimeForm.staff_id,
      ot_date: overtimeForm.ot_date,
      ot_start_time: overtimeForm.ot_start_time,
      ot_end_time: overtimeForm.ot_end_time,
      reason: overtimeForm.reason,
    };

    try {
      const response = await axios.post(
        `http://127.0.0.1:8000/hr/${type}/overtimes/`,
        requestData,
        { headers: { Authorization: `Bearer ${token}` } }
      );
      ...
      console.error("Error submitting overtime request:", err.response?.data);
      setError(
        err.response?.data?.detail ||
          err.response?.data?.non_field_errors?.[0] ||
  ...
  const handleSubmitEditOvertime = async (e) => {
    e.preventDefault();
    if (!token) {
      setError("Please log in to update overtime request.");
      return;
    }
    ...
    if (!editForm.ot_start_time || !editForm.ot_end_time) {
      setError("Start and end times are required.");
      return;
    }

    const start = new Date(`${editForm.ot_date}T${editForm.ot_start_time}`);
    let end = new Date(`${editForm.ot_date}T${editForm.ot_end_time}`);
    if (end < start) {
      end = new Date(end.getTime() + 24 * 60 * 60 * 1000);
    }
    if (end <= start) {
    ...
    const requestData = {
      ot_date: editForm.ot_date,
      ot_start_time: editForm.ot_start_time,
      ot_end_time: editForm.ot_end_time,
      reason: editForm.reason,
    };

    try {
      const response = await axios.patch(
        `http://127.0.0.1:8000/hr/${type}/overtimes/${selectedRequest.id}/`,
        requestData,
        { headers: { Authorization: `Bearer ${token}` } }
      );
      console.log("Overtime request updated:", response.data);
      for (const comment of editForm.comments) {
        if (comment.text.trim() && !comment.id) {
          try {
            await axios.post(
              `http://127.0.0.1:8000/hr/${type}/overtimes/${selectedRequest.id}/comments/`,
      ...
      const refreshResponse = await axios.get(
        staff_id
          ? `http://127.0.0.1:8000/hr/${type}/overtimes/?staff_id=${staff_id}`
          : `http://127.0.0.1:8000/hr/${type}/overtimes/`,
        { headers: { Authorization: `Bearer ${token}` } }
      );
      ...
      console.error("Error updating overtime request:", err.response?.data);
      setError(
        err.response?.data?.detail ||
          err.response?.data?.non_field_errors?.[0] ||
          Object.values(err.response?.data)?.[0] ||
          "Failed to update overtime request or post comment."
      );
    }
  };
    ...
    <div style={{ padding: "2rem", margin: "0 auto" }}>
      {staff_id ? (
        ...
        <>
          ...
          >
            Overtime Requests for {type === "staff" ? "Staff" : "Manpower"} ID: {staff_id}
          </h2>
          {error && <div style={errorStyle}>{error}</div>}
          {staffDetails && (
            <>
              {/* New Overtime Application Form */}
              <div style={sectionStyle}>
                ...
                <form onSubmit={handleSubmitNewOvertime}>
                  <div style={{ ...formRowStyle, alignItems: "center" }}>
                    ...
                    <div style={formColumnStyle}>
                      ...
                      <input
                        type="time"
                        name="ot_start_time"
                        value={overtimeForm.ot_start_time}
                        onChange={handleNewFormChange}
                        style={inputStyle}
                        required
                      />
                    ...
                    <div style={formColumnStyle}>
                      ...
                      <input
                        type="time"
                        name="ot_end_time"
                        value={overtimeForm.ot_end_time}
                        onChange={handleNewFormChange}
                        style={inputStyle}
                        required
                      />
                    ...
                    <div style={formColumnStyle}>
                      ...
                      <input
                        ...
                        value={calculateDuration(
                          overtimeForm.ot_start_time,
                          overtimeForm.ot_end_time,
                          overtimeForm.ot_date
                        )}
                        readOnly
                        style={inputStyle}
                      />
                  ...
                  <div style={sectionStyle}>
                    <label style={labelStyle}>Reason for Overtime</label>
                    <textarea
                      name="reason"
        ...
        <>
          <h2
            style={{ marginBottom: "1rem", color: "#333", fontSize: "1.5rem" }}
          >
            {type === "staff" ? "Staff" : "Manpower"} Overtime Requests
          </h2>
          {error && <div style={errorStyle}>{error}</div>}
          <div
            style={{
          ...
          >
            <div style={{ display: "flex", flexWrap: "wrap", gap: "1rem", alignItems: "center" }}>
              {[
                ...
                { name: "otDate", label: "Overtime Date", width: "150px", type: "date" },
            ...
            <div>
              <button
                ...
                onMouseOver={(e) => (e.target.style.backgroundColor = "#333")}
                onMouseOut={(e) => (e.target.style.backgroundColor = "#000")}
                aria-label="New overtime application"
              >
                New Overtime Application
              </button>
            </div>
      ...
      {/* New Overtime Modal (only when no staff_id) */}
          ...
          <div style={modalContentStyle}>
            ...
            <div style={viewFormStyle}>
              ...
              <div style={viewFormItemStyle}>
                ...
                <input
                  type="text"
                  value={selectedRequest.ot_start_time}
                  readOnly
                  style={inputStyle}
                />
              </div>
              <div style={viewFormItemStyle}>
                ...
                <input
                  type="text"
                  value={selectedRequest.ot_end_time}
                  readOnly
                  style={inputStyle}
                />
              </div>
      ...
      {showEditModal && selectedRequest && (
        <div style={modalStyle}>
          <div style={modalContentStyle}>
            ...
            <form onSubmit={handleSubmitEditOvertime}>
              <div style={{ ...formRowStyle, alignItems: "center" }}>
                <div style={formColumnStyle}>
                  <label style={labelStyle}>Staff Name</label>
                  <input
                ...
                <div style={formColumnStyle}>
                  ...
                  <input
                    type="text"
                    value={calculateDuration(
                      editForm.ot_start_time,
                      editForm.ot_end_time,
                      editForm.ot_date
                    )}
                    readOnly
                    style={inputStyle}
