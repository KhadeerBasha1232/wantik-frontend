...
const Fines = ({ staff_id }) => {
  ...
  useEffect(() => {
    if (!token) {
      ...
      setError("Please log in to view fine requests.");
      return;
    }
    ...
    if (staff_id) {
      ...
      // Fetch fine requests for specific staff
      axios
        .get(`http://127.0.0.1:8000/hr/${type}/fines/?staff_id=${staff_id}`, {
          headers: { Authorization: `Bearer ${token}` },
        })
        .then((res) => {
          console.log("Fine Requests:", res.data);
          const normalizedData = res.data.map((request) => ({
            ...request,
            request_date: normalizeDate(request.request_date),
          }));
          setFineRequests(normalizedData);
          setFilteredRequests(normalizedData);
        })
        .catch((err) => {
          console.error("Error fetching fine requests:", err.response?.data);
          setError("Failed to load fine requests.");
        });
    } else {
      ...
      // Fetch all fine requests
      axios
        .get(`http://127.0.0.1:8000/hr/${type}/fines/`, {
          headers: { Authorization: `Bearer ${token}` },
        })
        .then((res) => {
          console.log("Fine Requests:", res.data);
  ...
  useEffect(() => {
    setFilteredRequests(
      fineRequests.filter((request) => {
        const requestDate = new Date(request.request_date);
        const filterDate = filters.fineDate ? new Date(filters.fineDate) : null;
        ...
        return (
          ...
          (!filters.fineDate ||
            requestDate.toISOString().split("T")[0] === filters.fineDate)
        );
      })
    );
  }, [filters, fineRequests]);
  ...
  const deleteComment = async (index) => {
    ...
    if (comment.id) {
      try {
        await axios.delete(
          `http://127.0.0.1:8000/hr/${type}/fines/${selectedRequest.id}/comments/${comment.id}/`,
          { headers: { Authorization: `Bearer ${token}` } }
        );
  ...
  const handleStatusChange = async (requestId, field, value) => {
    ...
    try {
      const response = await axios.patch(
        `http://127.0.0.1:8000/hr/${type}/fines/${requestId}/`,
        { [field]: value },
        { headers: { Authorization: `Bearer ${token}` } }
      );
      console.log("Status updated:", response.data);
      setFineRequests((prev) =>
        prev.map((req) =>
          req.id === requestId
            ? {
                ...req,
  ...
  const handleSubmitNewFine = async (e) => {
    e.preventDefault();
    if (!token) {
      setError("Please log in to submit a fine request.");
      return;
    }

    if (!fineForm.staff_id) {
      setError("Please select a staff member.");
      return;
    }

    if (!fineForm.fine_amount || fineForm.fine_amount <= 0) {
    ...
    const requestData = {
      staff_id: fineForm.staff_id,
      fine_amount: parseFloat(fineForm.fine_amount),
      reason: fineForm.reason,
    };

    try {
      const response = await axios.post(
        `http://127.0.0.1:8000/hr/${type}/fines/`,
        requestData,
        { headers: { Authorization: `Bearer ${token}` } }
      );
      console.log("Fine request submitted:", response.data);
      setFineRequests((prev) => [
        ...prev,
        {
          ...response.data,
      ...
      console.error("Error submitting fine request:", err.response?.data);
      setError(
        err.response?.data?.detail ||
          err.response?.data?.non_field_errors?.[0] ||
          "Failed to submit fine request."
      );
    }
  };

  const handleSubmitEditFine = async (e) => {
    e.preventDefault();
    if (!token) {
      setError("Please log in to update fine request.");
      return;
    }

    if (!editForm.fine_amount || editForm.fine_amount <= 0) {
      setError("Fine amount must be greater than zero.");
      return;
    }
    ...
    try {
      const response = await axios.patch(
        `http://127.0.0.1:8000/hr/${type}/fines/${selectedRequest.id}/`,
        requestData,
        { headers: { Authorization: `Bearer ${token}` } }
      );
      console.log("Fine request updated:", response.data);
      for (const comment of editForm.comments) {
        if (comment.text.trim() && !comment.id) {
          try {
            await axios.post(
              `http://127.0.0.1:8000/hr/${type}/fines/${selectedRequest.id}/comments/`,
      ...
      const refreshResponse = await axios.get(
        staff_id
          ? `http://127.0.0.1:8000/hr/${type}/fines/?staff_id=${staff_id}`
          : `http://127.0.0.1:8000/hr/${type}/fines/`,
        { headers: { Authorization: `Bearer ${token}` } }
      );
      ...
      console.error("Error updating fine request:", err.response?.data);
      setError(
        err.response?.data?.detail ||
          err.response?.data?.non_field_errors?.[0] ||
          Object.values(err.response?.data)?.[0] ||
          "Failed to update fine request or post comment."
      );
    }
  };
    ...
    <div style={{ padding: "2rem", margin: "0 auto" }}>
      {staff_id ? (
        ...
        <>
          ...
          >
            Fine Requests for {type === "staff" ? "Staff" : "Manpower"} ID: {staff_id}
          </h2>
          {error && <div style={errorStyle}>{error}</div>}
          {staffDetails && (
            <>
              {/* New Fine Application Form */}
              <div style={sectionStyle}>
                <h3
                  style={{ fontSize: "1.2rem", marginBottom: "1rem", color: "#333" }}
                >
                  New Fine Application
                </h3>
                <form onSubmit={handleSubmitNewFine}>
                  ...
                  <div style={sectionStyle}>
                    <label style={labelStyle}>Reason for Fine</label>
                    <textarea
                      name="reason"
        ...
        <>
          <h2
            style={{ marginBottom: "1rem", color: "#333", fontSize: "1.5rem" }}
          >
            {type === "staff" ? "Staff" : "Manpower"} Fine Requests
          </h2>
          {error && <div style={errorStyle}>{error}</div>}
          <div
            style={{
          ...
          >
            <div style={{ display: "flex", flexWrap: "wrap", gap: "1rem", alignItems: "center" }}>
              {[
                ...
                { name: "fineDate", label: "Fine Date", width: "150px", type: "date" },
            ...
            <div>
              <button
                ...
                onMouseOver={(e) => (e.target.style.backgroundColor = "#333")}
                onMouseOut={(e) => (e.target.style.backgroundColor = "#000")}
                aria-label="New fine application"
              >
                New Fine Application
              </button>
            </div>
      ...
      {/* New Fine Modal (only when no staff_id) */}
          ...
          <div style={modalContentStyle}>
            ...
            <div style={viewFormStyle}>
              ...
              <div style={viewFormItemStyle}>
                ...
                <input
                  type="text"
                  value={selectedRequest.fine_amount}
                  readOnly
                  style={inputStyle}
                />
              </div>
      ...
      {showEditModal && selectedRequest && (
        <div style={modalStyle}>
          <div style={modalContentStyle}>
            ...
            <form onSubmit={handleSubmitEditFine}>
              <div style={{ ...formRowStyle, alignItems: "center" }}>
                <div style={formColumnStyle}>
                  <label style={labelStyle}>Staff Name</label>
                  <input
